...
\end{tikzpicture}
\end{center}
\caption{Graphical model of the PIGEAN framework showing the relationships between observed association data $\mathcal{D}_i$, latent disease-relevance indicators $d_i$, and annotation effect sizes $\beta_j$.}
...
\begin{itemize}
  ...
  \item We use an annotation specific variance $\sigma_j$ related to
    ...
    \item Analogous to the $w$ parameter, we use an $\alpha$ parameter
      ...
      reasons discussed below (Section~\ref{sec:simval}), genetic support is biased toward
...
same assumptions (exactly one causal SNP in a set), it has been shown\cite{} that the posterior probability that the causal gene is gene $i$
...
\begin{itemize}
  ...
  \item If the marginal $\tilde{\beta}$ for the gene annotation
    ...
    annotation. Empirically, for reasons discussed below (Section~\ref{sec:simval}),
...
\subsection{Alternative Trait Correlation Correction Methods}

In addition to inverse correlation weighting (the primary method), we evaluated three alternative approaches for correcting trait redundancy in pleiotropy estimates.

\paragraph{Uncorrected (raw) summation.}
Each trait receives equal weight ($w_t = 1$), providing no correction for trait correlations. This serves as a baseline and is expected to overestimate pleiotropy for genes associated with clusters of genetically correlated phenotypes.

\paragraph{PCA-based weighting.}
Trait weights are derived from the loadings of principal components of the trait correlation matrix. We retained components explaining 90\% of variance (1,038 components for combined evidence, 1,166 for direct evidence, and 609 for indirect evidence out of 3,026 traits). PCA weights showed low variance (standard deviation 0.063--0.090), indicating that this method distributes weight relatively evenly across traits compared to inverse correlation weighting (standard deviation 0.73--0.82). We evaluated PCA weighting using both the trait correlation matrix and the gene--trait score matrix as input, obtaining nearly identical results.

\paragraph{Whitening transformation.}
Trait weights are derived from the matrix square root of the inverse correlation matrix ($C^{-1/2}$), which transforms the correlated trait space into an uncorrelated one. This method produced the highest weight variance (standard deviation 1.2--1.8), reflecting aggressive decorrelation. Condition numbers of the correlation matrices were large ($59$--$152 \times 10^{6}$), necessitating eigenvalue regularization (minimum eigenvalue $10^{-6}$) for numerical stability. While whitening provides theoretically optimal decorrelation, its sensitivity to near-singular matrices makes it less robust than inverse correlation weighting in practice.

\paragraph{Correlation evidence type.}
Independently of the correction method, we varied which type of PIGEAN evidence was used to compute the trait--trait correlation matrix (the correlation evidence type), distinct from the evidence type used to compute pleiotropy itself. We evaluated all nine combinations of pleiotropy evidence type $\times$ correlation evidence type. The choice of correlation evidence type had minimal impact on the constraint--pleiotropy relationship, with the primary setting (correlation evidence type = direct) being representative (Extended Data Table~\ref{tab:pleiotropy_constraint}).

\paragraph{Sensitivity to analytical choices.}
The constraint--pleiotropy association was robust across all correction methods and parameter settings. Combined evidence generally yielded the strongest constraint correlations (LOEUF $\rho = -0.261$ across all traits), followed by direct evidence ($\rho = -0.252$) and indirect evidence ($\rho = -0.214$). Both direct and indirect evidence are expected to correlate with constraint, as they capture complementary modes by which constrained genes manifest pleiotropic effects---through direct genetic associations and through membership in trait-associated pathways, respectively. Portal traits consistently showed the strongest effects (combined LOEUF $\rho = -0.247$), likely reflecting greater GWAS power for common diseases. For GWAS catalog traits, indirect evidence showed stronger constraint correlations than direct evidence (LOEUF $\rho = -0.233$ vs.\ $-0.155$), suggesting that pathway-mediated evidence is relatively more informative for trait sets with weaker individual GWAS signals. The synonymous z-score showed near-zero correlations across all settings ($|\rho| < 0.05$), serving as a consistent negative control.
